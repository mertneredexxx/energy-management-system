...
const MeteorologicalDatas = () => {
  ...
  const upsertSetting = async (changes) => {
    ...
    await supabase.from('home_settings').upsert({ user_id: user.id, ...settings, ...changes }, { onConflict: 'user_id' });
  };
          ...
          <Card>
            <CardContent>
              ...
              <FormControl fullWidth sx={{ mt: 2 }}>
                ...
                <RadioGroup row value={settings.energy_type} onChange={(e) => upsertSetting({ energy_type: e.target.value })}>
              ...
              <FormControl fullWidth sx={{ mt: 2 }}>
                ...
                <RadioGroup row value={settings.duration} onChange={(e) => upsertSetting({ duration: e.target.value })}>
              ...
              <FormControl fullWidth sx={{ mt: 2 }}>
                ...
                <RadioGroup row value={settings.season} onChange={(e) => upsertSetting({ season: e.target.value })}>
              ...
              <Button variant="contained" sx={{ mt: 3 }} onClick={fetchMeteorologicalData}>
                Apply & Fetch Data
              </Button>
            </CardContent>
          </Card>
              ...
              <CardContent>
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <LineChart data={meteoData}>
                    ...
                    <YAxis unit={settings.energy_type === 'solar' ? ' MJ/m²' : ' m/s'} />
                    <Tooltip />
                    <Legend />
                    <Line
                      type="monotone"
                      dataKey="value"
                      name={settings.energy_type === 'solar' ? 'Irradiance' : 'Wind Speed'}
                      dot={false}
                      stroke={settings.energy_type === 'solar' ? '#f5a623' : '#357edd'}
                    />
                  </LineChart>
                </ResponsiveContainer>
              </CardContent>
