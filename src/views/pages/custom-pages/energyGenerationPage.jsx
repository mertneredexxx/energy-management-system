...
import {
    ...
    TextField, Button, Table, TableBody, TableCell, TableHead,
    TableRow, IconButton, MenuItem, Select, Grid,
    Box
} from '@mui/material';
...
export default function EnergyGeneration() {
    ...
    const [energyGen, setEnergyGen] = useState({         // form values
        catalogId: '',
        ratedPower: ''
    });
    ...
    useEffect(() => {
        (async () => {
            const { data: cat } = await supabase
                ...
                .select('*')
                .order('renewable_type, default_kw');
            setCatalog(cat);
            fetchSources();
        })();
    }, []);
    ...
    function buildHourlySeries(shape, ratedKw, renewableType) {
        ...
        const WIND_CF    = 0.30;  // average annual capacity factor for small turbines
    ...
    const handleRatedPowerChange = (e) =>
        setEnergyGen(prev => ({ ...prev, ratedPower: e.target.value }));
    ...
    return (
        <Card>
            ...
            <Dialog
                open={openChart.open}
                onClose={() => setOpenChart({ ...openChart, open: false })}
                maxWidth="md"
                fullWidth
            >
