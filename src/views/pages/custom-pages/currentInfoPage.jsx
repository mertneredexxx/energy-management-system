...
export default function InfoPage() {
  ...
  const [WIND_CF, setWindCf] = useState(0.3);
  ...
  async function saveReport() {
    ...
    const { data: hs, error: hsErr } = await supabase.from('home_settings').select('*').eq('user_id', user.id).single();
    if (hsErr) {
      console.error(hsErr);
  ...
  async function reload() {
    ...
    /* threshold */
    const { data: hs } = await supabase.from('home_settings').select('*').eq('user_id', user.id).single();
    ...
    season === 'summer' ? setWindCf(0.25) : setWindCf(0.3);
    ...
    const lookup = {};
    meteoRows.forEach((r) => {
      if (r.data_type === 'Solar' && r.duration === 'One Month') lookup.avgIrrMonth = r.data.average_irradiance;
      if (r.data_type === 'Wind' && r.duration === 'One Month') lookup.avgWindMonth = r.data.average_wind_speed;
      if (r.data_type === 'Solar' && r.duration === 'One Year') lookup.annIrrYear = r.data.annual_irradiance;
      if (r.data_type === 'Wind' && r.duration === 'One Year') lookup.annWindYear = r.data.annual_wind_speed;
    });
    setMeteoAvg(lookup);
    ...
    const costBefore = baseSeries.reduce(
      (sum, { Load, Production, hour }) => sum + Math.max(Load - Production, 0) * priceHr[+hour.slice(0, 2)],
      0
    );
  ...
  async function runShifting() {
    ...
    const flex = loadRows.filter((l) => l.priority > 1 && !l.is_all_day).sort((a, b) => b.priority - a.priority);
    ...
    for (let l of flex) {
      ...
      // re-insert at 22:00
      addInterval(shiftedHr, { power_rate: l.power_rate, start: newStart, end: newEnd }, +1);
      updates.push({
        id: l.id,
        start: '22:00',
  ...
  async function applyShift() {
    if (!preview) return;
    await Promise.all(preview.updates.map((u) => supabase.from('loads').update({ start_time: u.start, end_time: u.end }).eq('id', u.id)));
            ...
            <CardContent>
              ...
              <ResponsiveContainer width="100%" height={320}>
                <LineChart data={hourSeries}>
                            ...
                            {payload.map((entry, index) => (
                              <Box key={index} sx={{ display: 'flex', alignItems: 'center', mb: 0.5 }}>
                                ...
                                <Typography variant="body2" sx={{ fontWeight: 500 }}>
                                  {entry.dataKey}: {entry.value?.toFixed(2)} kW
                                </Typography>
                              </Box>
                            ))}
                          </Box>
                  ...
                  />
                  <Legend />
                  <Line type="monotone" dataKey="Load" stroke="#f44336" dot={false} strokeWidth={2} name="Load" />
                  <Line type="monotone" dataKey="ShiftedLoad" stroke="#ff9800" dot={false} strokeWidth={2} name="Load Shifting" />
                  <Line type="monotone" dataKey="EV" stroke="#2196f3" dot={false} strokeWidth={2} name="EV Charging" />
                  <Line type="monotone" dataKey="Production" stroke="#4caf50" dot={false} strokeWidth={2} name="Production" />
              ...
              {preview && (
                <Grid item xs={12} sx={{ textAlign: 'right', my: 2 }}>
                  <Button variant="contained" color="secondary" onClick={openReportDialog} disabled={saving}>
