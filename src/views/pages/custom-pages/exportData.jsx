...
export default function ExportPage() {
    ...
    (async () => {
      ...
      const [lRes, rRes, mRes, evRes] = await Promise.all([
        ...
        supabase
          ...
          .select(`*, catalog:renewable_catalog(model_name, renewable_type, hourly_shape)`)
          .eq('user_id', user.id),
      ...
      const dp = rRes.data.map((r) => {
        ...
        const daily = type === 'PV' ? rated * SUN_HOURS * PR : rated * WIND_CF * 24;
        prodTotal += daily;
        // distribute by shape
      ...
      const prodTbl = sels.map((r) => {
        ...
        const dailyKwh = type === 'PV' ? rated * SUN_HOURS * PR : rated * WIND_CF * 24;
  ...
  async function runShifting() {
    ...
    const capacity = hourSeries.map((p) => GRID_LIMIT + p.Production - Math.max(0, p.EV));
    ...
    for (const l of flexLoads) {
      ...
      for (let tryStart = 0; tryStart < 24; tryStart++) {
        ...
        for (let h = 0; h < duration; h++) {
          ...
          if (shiftedLoadHr[idx] + p_kW > capacity[idx]) {
            fits = false;
            break;
          }
        }
        ...
        addInterval(shiftedLoadHr, { power_rate: l.power_rate, start: tryStart, end: (tryStart + duration) % 24 }, +1);
    ...
    const costAfter = previewSeries.reduce(
      (sum, { ShiftedLoad, Production, hour }) => sum + Math.max(ShiftedLoad - Production, 0) * priceHr[+hour.slice(0, 2)],
      0
    );
      ...
      <Container id="export-page" sx={{ py: 4 }}>
        ...
        <Grid container spacing={2} sx={{ mb: 3 }}>
          <Grid item xs={12} md={6}>
            <TextField label="Report Title" fullWidth value={title} onChange={(e) => setTitle(e.target.value)} />
          </Grid>
          <Grid item xs={12} md={6}>
            <TextField
              label="Description"
                ...
                <TableContainer component={Paper} sx={{ mt: 2, mb: 4 }}>
                  <Table size="small">
                    <TableHead>
                      <TableRow>
                        <TableCell>
                          <strong>Charging Rate (kW)</strong>
                        </TableCell>
                        <TableCell>
                          <strong>Plug-in Time</strong>
                        </TableCell>
                        <TableCell>
                          <strong>Plug-out Time</strong>
                        </TableCell>
                        <TableCell>
                          <strong>Initial SoC (%)</strong>
                        </TableCell>
                        <TableCell>
                          <strong>Final SoC (%)</strong>
                        </TableCell>
                      </TableRow>
                    </TableHead>
                    <TableBody>
