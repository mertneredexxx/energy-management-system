...
export default function ReportsPage() {
  ...
  function diffReports(a, b) {
    ...
    const diffs = [];
    if (a.season !== b.season) diffs.push(`• Season: **${a.season}** → **${b.season}**`);
    if (+a.threshold_kw !== +b.threshold_kw) diffs.push(`• Threshold: **${a.threshold_kw} kW** → **${b.threshold_kw} kW**`);
    if (a.costs?.before !== b.costs?.before)
      diffs.push(`• Cost before shift: ₺${a.costs.before.toFixed(2)} → ₺${b.costs.before.toFixed(2)}`);
    if (a.costs?.after !== b.costs?.after) diffs.push(`• Cost after shift: ₺${a.costs.after.toFixed(2)} → ₺${b.costs.after.toFixed(2)}`);
    ...
    for (let name of new Set([...Object.keys(mapA), ...Object.keys(mapB)])) {
      ...
      if (!la) {
        ...
        diffs.push(`• **${name}**: ${la.start_time}–${la.end_time} → ${lb.start_time}–${lb.end_time}`);
      }
    }
  ...
  return (
    <>
                  ...
                  <FormControlLabel
                    control={
                      <Checkbox
                        ...
                        disabled={!compareReports.some((r) => r.id === rep.id) && compareReports.length >= 2}
                      />
                    }
                    label="Select"
                  />
      ...
      <CompareModal open={compareOpen} onClose={() => setCompareOpen(false)} reportA={compareReports[0]} reportB={compareReports[1]} />
      {/* Details Modal */}
      <Modal open={Boolean(detailReport)} onClose={() => setDetailReport(null)}>
        <Box sx={modalStyle}>
          {detailReport && (
            <>
              ...
              <Typography gutterBottom>
                Cost before: ₺{detailReport.costs?.before?.toFixed(2) || '—'}, after: ₺{detailReport.costs?.after?.toFixed(2) || '—'}
              </Typography>
              <Divider sx={{ my: 2 }} />
