...
const LoadsScreen = () => {
  ...
  useEffect(() => {
    const fetchScheduledLoads = async () => {
      if (!user) return;
      const { data, error } = await supabase.from('loads').select('*').eq('user_id', user.id);
      if (error) {
        console.error('Error fetching loads:', error.message);
      } else {
  ...
  const handleScheduleSubmit = async () => {
    ...
    if (!user || !selectedDevice || (!isAllDay && (startTime === '' || endTime === ''))) {
  ...
  const getPriorityStars = (priority) => {
    if (priority >= 1 && priority <= 3) {
      return Array.from({ length: 4 - priority }, (_, i) => <StarIcon key={i} sx={{ color: '#FFD700', fontSize: '1.2em' }} />);
    }
    return priority;
  };
                  ...
                  <Stack spacing={2}>
                    ...
                    >
                      <Box sx={{ display: 'flex', alignItems: 'center', width: '100%' }}>
                        ...
                        <Box sx={{ display: 'flex', alignItems: 'center' }}>{getPriorityStars(device.priority)}</Box>
                      </Box>
                    </Box>
                  </Stack>
                </CardContent>
            ...
            <Stack spacing={3}>
              ...
              >
                <FormControlLabel
                  ...
                  label={<Typography sx={{ fontWeight: 500 }}>Run all day</Typography>}
                />
              </Box>
